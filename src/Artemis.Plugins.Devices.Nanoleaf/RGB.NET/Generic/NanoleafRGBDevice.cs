...
{
    internal NanoleafRGBDevice(NanoleafRGBDeviceInfo deviceInfo, string address, ushort port, IDeviceUpdateTrigger updateTrigger) 
        : base(deviceInfo, new NanoleafDeviceUpdateQueue(updateTrigger, address, port, deviceInfo.Info.PanelLayout.Layout.NumPanels, deviceInfo.Info.PanelLayout.Layout.PositionData[0].ShapeType.GetExtControlVersion()))
    {
        InitializeLayout();
    }
